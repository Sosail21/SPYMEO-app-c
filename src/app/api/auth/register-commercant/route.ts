// Cdw-Spm: Commercant Registration API
...
const registerCommercantSchema = z.object({
  ...
  productNature: z.string().min(2, 'Nature des produits requise'),
...
export async function POST(request: NextRequest) {
  try {
    ...
    const data = registerCommercantSchema.parse(body);
    ...
    console.log(`[REGISTER-COMMERCANT] Nouvelle candidature: ${user.email}`);

    // Envoyer email à l'admin
    try {
      await sendEmail({
        to: process.env.ADMIN_EMAIL || 'cindy-dorbane@spymeo.fr',
        subject: `Nouvelle candidature Commerçant - ${data.firstName} ${data.lastName}`,
        html: emailTemplates.adminNotificationCommercant({
          firstName: data.firstName,
          lastName: data.lastName,
          email: data.email,
          phone: data.phone,
      ...
      console.error('[REGISTER-COMMERCANT] Erreur envoi email admin:', emailError);
      // Continue anyway
    }
    ...
    try {
      ...
      console.error('[REGISTER-COMMERCANT] Erreur envoi email candidat:', emailError);
      // Continue anyway
    }
    ...
    console.error('[REGISTER-COMMERCANT] Erreur:', error);
