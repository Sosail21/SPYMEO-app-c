// Cdw-Spm: Artisan Registration API
...
export async function POST(request: NextRequest) {
  try {
    ...
    const data = registerArtisanSchema.parse(body);
    ...
    console.log(`[REGISTER-ARTISAN] Nouvelle candidature: ${user.email}`);

    // Envoyer email à l'admin
    try {
      await sendEmail({
        to: process.env.ADMIN_EMAIL || 'cindy-dorbane@spymeo.fr',
        subject: `Nouvelle candidature Artisan - ${data.firstName} ${data.lastName}`,
        html: emailTemplates.adminNotificationArtisan({
          firstName: data.firstName,
          lastName: data.lastName,
          email: data.email,
          phone: data.phone,
      ...
      console.error('[REGISTER-ARTISAN] Erreur envoi email admin:', emailError);
      // Continue anyway
    }
    ...
    try {
      ...
      console.error('[REGISTER-ARTISAN] Erreur envoi email candidat:', emailError);
      // Continue anyway
    }
    ...
    console.error('[REGISTER-ARTISAN] Erreur:', error);
