// src/app/pro/formations/nouvelle/page.tsx
"use client";
...
export default function NewTrainingPage() {
  ...
  // auto-slug depuis le titre (modif manuelle possible)
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const r = await fetch("/api/center/trainings", {
        method: "POST",
      ...
      if (!r.ok) {
        // fallback mock success pour dev
        await new Promise((res) => setTimeout(res, 600));
      }

      // redirige vers la liste
      router.push("/pro/formations");
    } catch (err: any) {
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-600">
              Renseignez les informations principales. Vous pourrez ajouter des sessions ensuite.
            </p>
          </div>
          <div className="flex gap-2">
            <Link href="/pro/formations" className="btn btn-outline">Retour à la liste</Link>
          </div>
        </div>
      </section>
      ...
      <section className="section">
        <form onSubmit={onSubmit} className="grid gap-4">
          <div className="soft-card p-4 grid md:grid-cols-2 gap-4">
            <label className="grid gap-1">
              ...
              <input
                ...
                placeholder="Kobido — fondamentaux & posture"
              />
            </label>
            ...
            <label className="grid gap-1">
              <span className="text-sm text-muted">Modalité</span>
              <select
                value={form.modality}
                onChange={(e) => setForm({ ...form, modality: e.target.value as TrainingModality })}
                className="rounded-xl border border-slate-200 px-3 py-2"
              >
            ...
            <label className="grid gap-1 md:col-span-2">
              ...
              <textarea
                ...
                rows={4}
                className="rounded-xl border border-slate-200 px-3 py-2"
                placeholder="Objectifs pédagogiques, prérequis, modules, évaluation…"
              />
            </label>
            ...
            <div className="grid md:grid-cols-2 gap-4 md:col-span-2">
              ...
              <label className="grid gap-1">
                <span className="text-sm text-muted">Première session (optionnel)</span>
                <input
                  type="date"
                  value={form.firstSession ?? ""}
                  onChange={(e) => setForm({ ...form, firstSession: e.target.value })}
                  className="rounded-xl border border-slate-200 px-3 py-2"
                />
              </label>
            </div>
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Link href="/pro/formations" className="btn btn-outline">Annuler</Link>
          </div>
        </form>
      </section>
    </main>
