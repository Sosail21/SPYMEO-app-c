// src/app/pro/formations/[slug]/page.tsx
"use client";
    ...
    <main className="container-spy space-y-4 compact-sections">
      ...
      <section className="section">
        <div className="soft-card p-5">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
            <div>
              ...
              <p className="text-slate-600">
                {data ? (
                  <>
                    {labelModality(data.modality)} • {totalSessions} session{totalSessions > 1 ? "s" : ""} • {totalEnrolled} inscrit{totalEnrolled > 1 ? "s" : ""}
                    {nextSession && <> • Prochaine : {fmtDate(nextSession.date)}</>}
                  </>
                ) : "—"}
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
              <Link href={`/formations/${slug}`} target="_blank" className="pill pill-muted">
                Aperçu public
              </Link>
              <Link href={`/pro/formations/sessions`} className="pill pill-ghost">
          ...
          {data?.description && (
            <p className="mt-3 text-sm text-slate-700 whitespace-pre-wrap">{data.description}</p>
          )}

          <div className="mt-4 flex flex-wrap gap-2">
            <Link href={`/pro/formations/${slug}/sessions/nouvelle`} className="btn">
              + Créer une session
            </Link>
            {nextSession && (
              <Link href={`/pro/formations/sessions/${nextSession.id}`} className="btn btn-outline">
                Gérer la prochaine session
              </Link>
            )}
          </div>
        </div>
              ...
              return (
                <li key={s.id} className="soft-card p-4">
                  <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
                      ...
                      <div className="mt-1 text-sm text-slate-600 flex flex-wrap items-center gap-3">
                        {typeof seats === "number" ? (
                          <span className="inline-flex items-center gap-1">
                            <span>👥</span> {enrolled}/{seats} inscrits
                          </span>
                        ) : (
                          <span className="inline-flex items-center gap-1">
                            <span>👥</span> {enrolled} inscrit{enrolled > 1 ? "s" : ""}
                          </span>
                        )}
                        {typeof s.price === "number" && (
                          <span className="inline-flex items-center gap-1">
                            <span>💶</span> {s.price.toFixed(0)} € TTC
                          </span>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      <Link
                        href={`/pro/formations/sessions/${s.id}`}
                        className="pill pill-ghost"
                        title="Gérer la session (inscriptions, présence, actions)"
                      >
                        Gérer
                      </Link>
                      <Link
                        href={`/pro/apprenants/nouveau?sessionId=${s.id}`}
                        className="pill pill-ghost"
                        title="Inscrire un nouvel apprenant à cette session"
                      >
                        + Nouvel apprenant
                      </Link>
                      <button
                        ...
                        onClick={() => alert("Contacter les inscrits (à implémenter)")}
                        title="Envoyer un email aux inscrits"
                      >
                        Joindre les inscrits
                      </button>
                    </div>
                  </div>
                </li>
