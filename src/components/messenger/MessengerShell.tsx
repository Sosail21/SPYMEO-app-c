...
// ==== MOCK DATA (à remplacer par votre API plus tard) ====
...
export default function MessengerShell() {
  ...
  function send(text: string) {
    ...
    setConversations((prev) =>
      prev.map((c) => (c.id === activeId ? { ...c, lastMessage: text, lastAt: msg.createdAt } : c))
    );
  }

  function onSearch(q: string) {
    const s = q.trim().toLowerCase();
    if (!s) return setConversations(INITIAL_CONV);
    setConversations(
      INITIAL_CONV.filter(
        (c) =>
  ...
  return (
    <div className="h-[calc(100vh-56px)] bg-[#f7fbfd]">
      <div className="grid md:grid-cols-[340px_minmax(0,1fr)_300px] h-full">
        {/* Liste des conversations (drawer sur mobile) */}
        <aside
          className={`bg-white border-r border-border md:static fixed inset-y-0 left-0 z-40 w-[88%] max-w-[360px] transition-transform
        ...
        <main className="min-w-0 flex flex-col">
          <div className="md:hidden flex items-center justify-between p-3 bg-white border-b border-border sticky top-0 z-30">
            <button className="btn btn-ghost px-3" onClick={() => setMobileListOpen((v) => !v)}>
              ☰
            </button>
            <div className="font-semibold truncate">{activeConv?.with.name ?? "…"}</div>
            <button className="btn btn-ghost px-3" onClick={() => setShowInfo((v) => !v)}>
              ℹ️
            </button>
          </div>
        ...
        {/* Panneau d’info */}
        <aside
          className={`bg-white border-l border-border md:static fixed inset-y-0 right-0 z-40 w-[86%] max-w-[320px] transition-transform
