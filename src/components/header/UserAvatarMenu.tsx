...
export function UserAvatarMenu({ name }: { name?: string }) {
      ...
      {open && (
        <div
          ...
          className="
            ...
            bg-white border border-border rounded-xl shadow-[0_10px_30px_rgba(11,18,57,0.12)]
            p-2 grid gap-1 z-50
          "
        >
          <Link
          ...
          <form role="menuitem" action="/api/auth/logout" method="POST">
            <button className="page w-full text-left hover:bg-[#f7fbfd]" type="submit">
              Se déconnecter
            </button>
          </form>
        </div>
